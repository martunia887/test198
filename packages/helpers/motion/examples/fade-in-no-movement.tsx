...
export default () => {
  ...
  return (
    <div>
        ...
        >
          <StaggeredEntrance columns="responsive">
            {Array(state.numOfChildren)
              .fill(undefined)
              .map((_, index) => (
                <FadeIn disableMovement={movementDisabled} key={index}>
                  {props => (
                    <li
                      {...props}
      ...
      <Centered css={{ marginTop: '20px' }}>
        <h4>You can disable movement when fading elements and out</h4>
      </Centered>
      <Centered>
        <p>
          Movements are <b>{movementDisabled ? 'Disabled' : 'Enabled'}</b>
        </p>
        <Toggle
          onChange={() => setMovementDisabled(!movementDisabled)}
