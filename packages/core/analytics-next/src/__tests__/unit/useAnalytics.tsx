...
} from '../../withAnalyticsHook';

interface WrappedProps extends WithAnalyticsHookProps {
  abc?: string;
...
it('should render the provided component', () => {
  const ButtonWithAnalytics = withAnalyticsHook()(Button);
...
it('should have descriptive displayName', () => {
  const ButtonWithAnalytics = withAnalyticsHook()(Button);
  expect(ButtonWithAnalytics.displayName).toBe('WithAnalyticsHook(Button)');
});
...
describe('createAnalyticsEvent function prop', () => {
  it('should return a new UI Analytics Event with specified action and payload', () => {
    ...
    const ButtonWithAnalytics = withAnalyticsHook()(ButtonWithCreate);
    const wrapper = mount(
      <ButtonWithAnalytics
...
describe('create event map', () => {
  it('should patch callback props to create an event when passed a string', () => {
    let analyticsEvent: UIAnalyticsEvent | undefined;
    const ButtonWithAnalytics = withAnalyticsHook({
...
it('should not update patched prop callbacks across renders when the original callback has not changed', () => {
  interface ChangeCounterProps extends WithAnalyticsHookProps {
    onClick: () => void;
  }
  ...
  const ChangeCounterWithAnalytics = withAnalyticsHook({
    onClick: {
      action: 'clicked',
      time: Date.now(),
    },
