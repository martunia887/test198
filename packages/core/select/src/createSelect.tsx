...
import { mergeStyles, makeAnimated, AsyncCreatable } from 'react-select';
...
interface ReactSelectProps {
  ...
  filterOption: ((options: {}, filter: string) => boolean) | null;
  /* Formats option labels in the menu and control as React components */
  formatOptionLabel?: (options: OptionType, {}) => Node;
  /* Resolves option data to a string to be displayed as the label by components */
  getOptionLabel: (options: OptionType) => string;
  /* Resolves option data to a string to compare options and specify value attributes */
  getOptionValue: (options: OptionType) => string;
  ...
  loadingMessage?: ({ inputValue }: { inputValue: string }) => string;
  ...
  onChange?: (value: ValueType, options: Array<OptionType>) => void;
...
interface StylesFlags {
  isFocused?: boolean;
  isDisabled?: boolean;
  isSelected?: boolean;
}

function baseStyles(validationState: ValidationState, isCompact: boolean) {
  return {
    control: (
      css: React.CSSProperties,
      { isFocused, isDisabled }: StylesFlags,
    ) => {
    ...
    valueContainer: (css: React.CSSProperties) => ({
      ...css,
      paddingBottom: isCompact ? 0 : 2,
      paddingTop: isCompact ? 0 : 2,
    }),
    clearIndicator: (css: React.CSSProperties) => ({
      ...css,
      color: colors.N70,
      paddingLeft: '2px',
      paddingRight: '2px',
    ...
    loadingIndicator: (css: React.CSSProperties) => ({
      ...css,
      paddingBottom: isCompact ? 0 : 6,
      paddingTop: isCompact ? 0 : 6,
    }),
    ...
    option: (
      css: React.CSSProperties,
      { isFocused, isSelected }: StylesFlags,
    ) => {
    ...
    placeholder: (css: React.CSSProperties) => ({ ...css, color: colors.N100 }),
    singleValue: (css: React.CSSProperties, { isDisabled }: StylesFlags) => ({
      ...css,
      color: isDisabled ? colors.N70 : colors.N800,
      lineHeight: `${gridSize() * 2}px`, // 16px
    }),
    menuList: (css: React.CSSProperties) => ({
      ...css,
      paddingTop: gridSize(),
      paddingBottom: gridSize(),
    }),
    multiValue: (css: React.CSSProperties) => ({
      ...css,
      borderRadius: '2px',
    ...
    multiValueLabel: (css: React.CSSProperties) => ({
      ...css,
      padding: '2px',
      paddingRight: '2px',
    }),
...
export default function createSelect(WrappedComponent: ComponentType<any>) {
  return class AtlaskitSelect extends Component<SelectComponentsProps> {
    components?: {};
    ...
    onSelectRef = (ref: HTMLSelectElement) => {
      this.select = ref;
