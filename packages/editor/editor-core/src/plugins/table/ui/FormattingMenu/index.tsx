...
import { FormattingMenuProps, Rule, FormattingMenuState } from './types';
...
export default class FormattingMenu extends React.Component<
  ...
  FormattingMenuState
> {
  constructor(props: FormattingMenuProps) {
    super(props);
    this.state = getDefaultState();
  }

  // getting tableHeader cell formatting
  componentDidUpdate(nextProps: FormattingMenuProps) {
                ...
                {this.state.isColorPickerOpen && (
                  <div className={ClassName.FORMATTING_COLOR_PICKER}>
                    <ColorPalette
                      ...
                      selectedColor={this.state.background || null}
  ...
  private handleSave = () => {
    ...
    const formatting = {
      rules,
      marks: rules.length ? this.state.marks : [],
      background: rules.length ? this.state.background : undefined,
    };
