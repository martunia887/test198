...
const toSelectItems = (arr?: Array<any> | null) =>
  arr &&
  arr.map(item => ({
    label: item.title,
    value: item.id,
...
export default class RefsMenu extends React.Component<
  ...
  RefsMenuState
> {
  constructor(props: RefsMenuProps) {
    super(props);
    ...
    props.provider.getReferences().then((references: Array<Reference>) => {
      this.setState({
        tables: references,
        loading: false,
      });
  ...
  private handleSelectTable = ({ label, value }: any) => {
    this.setState({
      selectedTableId: value,
    });
  };

  private handleSelectColumn = ({ label, value }: any) => {
    this.setState({
