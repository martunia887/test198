...
export function ancestorListDepth(pos: ResolvedPos, nodes): number | undefined {
  ...
  const closestOrdered = findParentNodeOfTypeClosestToPos(pos, orderedList);
  const closestBullet = findParentNodeOfTypeClosestToPos(pos, bulletList);

  if (closestOrdered && closestBullet) {
    return Math.max(closestOrdered.depth, closestBullet.depth);
  } else if (closestOrdered) {
    return closestOrdered.depth;
  } else if (closestBullet) {
    return closestBullet.depth;
  }
}
