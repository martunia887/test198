...
const insideJiraQueryView = (ref: HTMLElement): boolean => {
  while (ref) {
    if (ref.classList && ref.classList.contains('jiraQueryView-content-wrap')) {
      return true;
    }
    ref = ref.parentNode as HTMLElement;
  }
...
export default class ClickAreaBlock extends React.Component<Props> {
  private handleClick = event => {
    ...
    if (insideJiraQueryView(event.target.parentNode)) {
      return;
    }
